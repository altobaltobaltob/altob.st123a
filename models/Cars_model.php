    ...
    public function save_db_io($parms, $rows, $opendoor=false)
	{
		...
		switch($parms['io'])
        {
			...
			case 'MO':
				...
				// 離場流程
                return $this->do_carout($rows, $parms, $rows_cario, $opendoor);
            	break;
        }
    }
	
	// 離場流程
	function do_carout($rows, $parms, $rows_cario, $opendoor, $skip_check=false)
	{
			...
			switch(true)
			{
				...
				case $co_time_minutes > 720 && !empty($rows_cario['payed']):
					...
					if($opendoor)
					{
						...
						// 產生回傳
						return $this->gen_return_msg($msg_id); // 2018/01/03 臨停不用放人
					}
					else
					{
						$data = array
